...
ggplot(district_crime_summary, aes(x = reorder(District, -Crime_Count), y = Crime_Count)) +
  ...
  theme_minimal()
```
###What are the most common environments for crime to take place?

``` {r}
library(dplyr)

# find the top 20 locations by crime
top_location_crimes <- crimes %>%
  group_by(Location.Description) %>%
  summarize(Crime_Count = n()) %>%
...
# visualize the top 20 locatiosn by crime
ggplot(top_location_crimes, aes(x = reorder(Location.Description, -Crime_Count), y = Crime_Count)) +
  geom_bar(stat = "identity", fill = "blue") +
  coord_flip() +
  labs(title = "Top 20 Locations with the Most Crimes", x = "Location", y = "Crime Count") +
  theme_minimal()
...
ggplot(street_crimes, aes(x = reorder(Block, -Crime_Count), y = Crime_Count)) +
  geom_bar(stat = "identity", fill = "blue") +
  coord_flip() +
  labs(title = "Top 20 Blocks with Most Street Crime", x = "Block", y = "Crime Count") +
  theme_minimal()
