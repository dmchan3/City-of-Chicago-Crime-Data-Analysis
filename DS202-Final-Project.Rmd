...
# Define the years and file parts
years <- 2020:2024
file_parts <- c("_2.csv")  # Include only "_2.csv" for each year

# Function to load and clean data
load_and_clean <- function(year, part) {
  # Dynamically construct the filename
  file <- paste0(year, part)
  
  # Skip loading 2020_1.csv as per requirement
  if (file == "2020_1.csv") {
    return(NULL)  # Return NULL to exclude this file
  }
  ...
  if (!"Location.Description" %in% colnames(data)) {
    stop(paste("Column 'Location.Description' not found in dataset:", file))
  }
  
  # Filter out rows with missing values in 'Location.Description'
  # and keep only the relevant column
  data <- data %>%
    filter(!is.na(Location.Description)) %>%  # Exclude rows with missing values
    select(Location.Description)  # Keep only the 'Location.Description' column
  
  return(data)  # Return the cleaned data
}

# Load and combine data from all files except 2020_1.csv
crime_data <- map_dfr(years, function(year) {
  map_dfr(file_parts, function(part) {
    load_and_clean(year, part)  # Apply the cleaning function to each file
  })
})

# Count occurrences by 'Location.Description'
locations_by_frequency <- crime_data %>%
  group_by(Location.Description) %>%  # Group by location
  summarize(
    Total_Reports = n()  # Count the total number of reports for each location
  ) %>%
  arrange(desc(Total_Reports))  # Sort by total reports in descending order

# Extract the top 10 locations with the most reports
top_locations <- locations_by_frequency %>%
  slice_max(order_by = Total_Reports, n = 10)  # Get the top 10 rows

# Print the top 10 locations
print("Top 10 Locations by Total Reports:")
print(top_locations)

# Visualization: Total reports by location (Top 10)
ggplot(top_locations, aes(x = reorder(Location.Description, -Total_Reports), y = Total_Reports)) +
  geom_bar(stat = "identity", fill = "steelblue") +  # Create a bar chart
  scale_y_continuous(labels = scales::comma) +      # Format y-axis labels with commas
  labs(
    title = "Top 10 Locations by Total Reports",    # Title of the plot
    x = "Location Description",                    # Label for x-axis
    y = "Total Reports"                            # Label for y-axis
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for readability
```

**Libraries**: I loaded the tidyverse library for data manipulation and visualization, as well as scales to format y-axis labels in the plot.

**File Loading**: I defined the years (2020–2024) and specified that only _2.csv files should be processed. I explicitly skipped 2020_1.csv within the load_and_clean function.

**Data Cleaning**: I ensured that the Location.Description column exists, filtered out rows with missing values in this column, and kept only the Location.Description column for further analysis.

**Data Aggregation**: I counted the total number of reports for each unique location and extracted the top 10 locations with the highest number of reports.

**Visualization**: I created a bar chart of the top 10 locations, with bars ordered by the total number of reports. I rotated the x-axis labels for better readability.

**Output**: I displayed the top 10 locations along with their respective counts and generated a visualization to clearly represent the data.
